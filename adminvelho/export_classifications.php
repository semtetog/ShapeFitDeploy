            ...
            <?php foreach ($foods as $food): ?>
                <div class="food-card" data-food-id="<?= $food['id'] ?>">
                    <div class="food-header">
                        ...
                        <div class="food-info">
                            <div class="food-name"><?= htmlspecialchars($food['name_pt']) ?></div>
...
// Alternar categoria (adicionar/remover)
function toggleCategory(foodId, category) {
    ...
    // Debug log
    console.log('Após toggle - Classificações:', classifications);
    console.log('Alimento', foodId, 'tem categorias:', classifications[foodId] || 'nenhuma');
...
function updateFoodVisual(foodId) {
    ...
    foodCard.querySelectorAll('.category-btn').forEach(btn => {
        ...
        if (foodCategories.includes(btnCategory)) {
    ...
    if (foodCategories.length === 0 || !classifications[foodId]) {
        ...
        categoryDisplay.textContent = 'Não classificado';
        categoryDisplay.style.background = '#ef444420';
        categoryDisplay.style.color = '#ef4444';
        categoryDisplay.style.borderColor = '#ef4444';
        ...
        if (unitsBtn) {
            ...
            if (hint) {
                hint.textContent = 'Salve a classificação primeiro';
            }
        }
        ...
        // Estado classificado
        const categoryNames = foodCategories.map(cat => window.categories[cat].name).join(', ');
        categoryDisplay.textContent = categoryNames;
        
        // Usar cor da primeira categoria como principal
        const firstCategory = foodCategories[0];
        const categoryData = window.categories[firstCategory];
        categoryDisplay.style.background = categoryData.color + '20';
        categoryDisplay.style.color = categoryData.color;
        categoryDisplay.style.borderColor = categoryData.color;
