...
$is_capacitor = (
    isset($_SERVER['HTTP_X_CAPACITOR']) || 
    (isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], 'Capacitor') !== false) ||
    (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'capacitor://') !== false) ||
    (isset($_SERVER['HTTP_ORIGIN']) && strpos($_SERVER['HTTP_ORIGIN'], 'capacitor://') !== false)
);

// URL da API sempre aponta para Hostinger (mesmo no app)
if (!defined('API_BASE_URL')) {
    define('API_BASE_URL', 'https://appshapefit.com');
}

// Se estiver no Capacitor, usar URLs relativas para páginas, mas API remota
if ($is_capacitor) {
    // No app nativo, páginas são servidas localmente (relativas)
    if (!defined('BASE_APP_URL')) {
        define('BASE_APP_URL', ''); // URLs relativas
    }
    if (!defined('BASE_ASSET_URL')) {
        define('BASE_ASSET_URL', ''); // Assets locais
    }
} else {
    // Se não estiver no Capacitor, usar a URL normal
