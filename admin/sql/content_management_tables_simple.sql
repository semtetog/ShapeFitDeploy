-- SQL SIMPLIFICADO para criar/corrigir tabelas de conteúdo
...
-- 2. Adicionar colunas que podem faltar (se a tabela já existir)
-- Verificar e adicionar file_path se não existir
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'sf_member_content' 
...
-- Verificar e adicionar file_size se não existir
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'sf_member_content' 
...
-- Verificar e adicionar mime_type se não existir
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'sf_member_content' 
